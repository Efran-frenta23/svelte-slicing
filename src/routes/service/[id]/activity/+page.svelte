<script>
    ...
    function viewActivityDetail(activity) {
        // Navigate to the existing service detail page
        window.location.href = `/Service-activity/Service-detail/${activity.id}`;
    }
</script>
...
<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <div>
            <button class="btn-back mb-2" on:click={() => history.back()} aria-label="Back to Service List">
                <i class="fas fa-arrow-left"></i> Back to Services
            </button>
            <h1 class="text-3xl font-bold text-yellow-800 flex items-center gap-3">
                <i class="fas fa-clipboard-list"></i>
                {serviceInfo ? `${serviceInfo.name || 'Service'} Activities` : 'Service Activity'}
            </h1>
            {#if serviceInfo}
                <p class="text-sm text-gray-500 mt-1">Service ID: {serviceId}</p>
            {/if}
        </div>
    </div>
                            ...
                            <tr class="border-b border-gray-200 hover:bg-yellow-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <button
                                        ...
                                        on:click={() => viewActivityDetail(activity)}
                                    >
